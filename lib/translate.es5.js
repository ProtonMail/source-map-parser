...
var inputSourceMap = [].concat(_toConsumableArray(process.argv)).filter(function () {
...
try {
  ...
  var sourceMaps = inputSourceMap.reduce(function (acc, key) {
    var _key$split = key.split('.'),
        _key$split2 = _slicedToArray(_key$split, 1),
        name = _key$split2[0];

    var _ref = name.match(/(appLazy|app)/) || [],
  ...
  stream.on('data', function (line) {
    ...
    var _row$split = row.split('.'),
        _row$split2 = _slicedToArray(_row$split, 1),
        name = _row$split2[0];

    var sourceMapApp = sourceMaps[name];
    ...
    if (!row || !sourceMapApp) {
      ...
      var pos = sourceMapApp.originalPositionFor({ line: _line, column: column }) || {};
